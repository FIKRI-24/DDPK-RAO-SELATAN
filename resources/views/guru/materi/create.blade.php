...
@section('title', 'Tambah Materi - Panel Guru')

@section('content')
<div class="row mb-4 justify-content-center">
    <div class="col-lg-8">
        <h2 class="fw-bold mb-0 text-dark">Tambah Materi Baru</h2>
        <p class="text-muted">Buat dan publikasikan materi pembelajaran baru untuk siswa Anda</p>
    </div>
</div>
            ...
            <div class="card-body p-4">
                <form action="{{ route('guru.materi.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control form-control-lg @error('judul') is-invalid @enderror" id="judul" name="judul" value="{{ old('judul') }}" required placeholder="Masukkan judul materi pembelajaran">
                        @error('judul')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control @error('isi_materi') is-invalid @enderror" id="isi_materi" name="isi_materi" rows="10" required placeholder="Tuliskan isi materi atau deskripsi lengkap materi disini...">{{ old('isi_materi') }}</textarea>
                        @error('isi_materi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        <label for="file_materi" class="form-label fw-bold d-flex align-items-center gap-1">
                            ...
                            <span>File Lampiran (Opsional)</span>
                        </label>
                        <input type="file" class="form-control @error('file_materi') is-invalid @enderror" id="file_materi" name="file_materi">
                        <div class="form-text text-muted small mt-2">
                            Tipe file yang diperbolehkan: <strong>pdf, doc, docx, ppt, pptx, jpg, jpeg, png</strong>. Maksimal 5 MB.
                        </div>
                        @error('file_materi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
