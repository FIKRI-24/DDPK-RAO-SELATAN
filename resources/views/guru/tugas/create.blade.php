...
@section('title', 'Tambah Tugas - Panel Guru')

@section('content')
<div class="row mb-4 justify-content-center">
    <div class="col-lg-8">
        <h2 class="fw-bold mb-0 text-dark">Tambah Tugas Baru</h2>
        <p class="text-muted">Buat tugas baru untuk dievaluasi oleh siswa</p>
    </div>
</div>
        ...
        <div class="card shadow-sm border-0" style="border-top: 3px solid #206bc4 !important;">
            <div class="card-body p-4">
                <form action="{{ route('guru.tugas.store') }}" method="POST">
                    ...
                    {{-- Pilihan Materi --}}
                    <div class="mb-4">
                        <label for="id_materi" class="form-label fw-bold d-flex align-items-center gap-1">
                        ...
                        <select class="form-select form-select-lg @error('id_materi') is-invalid @enderror" id="id_materi" name="id_materi" required>
                            <option value="" disabled {{ old('id_materi') ? '' : 'selected' }}>-- Pilih Materi --</option>
                            @foreach($materi as $item)
                                <option value="{{ $item->id_materi }}" {{ old('id_materi') == $item->id_materi ? 'selected' : '' }}>
                                    {{ $item->judul }}
                                </option>
                            @endforeach
                        </select>
                        <div class="form-text text-muted small mt-2">Tugas harus selalu dihubungkan ke salah satu materi ajar Anda.</div>
                        @error('id_materi')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" class="form-control form-control-lg @error('nama_tugas') is-invalid @enderror" id="nama_tugas" name="nama_tugas" value="{{ old('nama_tugas') }}" required placeholder="Masukkan nama atau judul tugas">
                        @error('nama_tugas')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea class="form-control @error('deskripsi') is-invalid @enderror" id="deskripsi" name="deskripsi" rows="8" placeholder="Tuliskan instruksi pengerjaan tugas di sini...">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
